...
class LlmcFp8Linear(nn.Module):
    ...
    def forward(self, x):
        ...
        y = torch.functional.F.linear(x, w, b)
        return y
...
class RotateLinear(nn.Module):
    ...
    def forward(self, x):
        ...
        w = self.weight.to(x.dtype)
        b = self.bias.to(x.dtype) if self.bias is not None else None
        x = torch.functional.F.linear(x, w, b)
...
class FakeQuantLinear(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.fp8_forward:
            ...
            w = self.tmp_weight.to(x.dtype)
            b = self.tmp_bias.to(x.dtype) if self.tmp_bias is not None else None
            y = torch.functional.F.linear(x, w, b)
        return y
